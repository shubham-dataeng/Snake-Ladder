 ...
 */
static const int DEFAULT_SNAKE_HEADS[] = {99, 94, 87, 76, 62, 47};
static const int DEFAULT_SNAKE_TAILS[] = {
    78, 56, 24, 58, 18, 26 /* tail < head always */
};
static const int DEFAULT_LADDER_BOTTOMS[] = {4, 9, 20, 28, 40, 51, 63, 71};
...
void board_init_defaults(Board *b) {
    ...
    for (int i = 0; i < NUM_DEFAULT_SNAKES; i++) {
        board_add_snake(b, DEFAULT_SNAKE_HEADS[i], DEFAULT_SNAKE_TAILS[i]);
    }
    /* Add default ladders */
    for (int i = 0; i < NUM_DEFAULT_LADDERS; i++) {
        board_add_ladder(b, DEFAULT_LADDER_BOTTOMS[i], DEFAULT_LADDER_TOPS[i]);
    }
...
bool board_add_snake(Board *b, int head, int tail) {
     ...
     */
    if (head <= tail) {
        fprintf(stderr, "Snake: head must be > tail\n");
        return false;
    }
    if (head < 2 || head > 99) {
        fprintf(stderr, "Snake head out of range\n");
        return false;
    }
    if (tail < 1 || tail > 98) {
        fprintf(stderr, "Snake tail out of range\n");
        return false;
    }
...
bool board_add_ladder(Board *b, int bottom, int top) {
    if (top <= bottom) {
        fprintf(stderr, "Ladder: top must be > bottom\n");
        return false;
    }
    if (bottom < 1 || bottom > 98) {
        fprintf(stderr, "Ladder bottom out of range\n");
        return false;
    }
    if (top < 2 || top > 100) {
        fprintf(stderr, "Ladder top out of range\n");
        return false;
    }
    ...
    for (int i = 0; i < b->num_snakes; i++) {
        ...
        for (int j = 0; j < b->num_ladders; j++) {
            if (b->ladders[j].bottom == head) {
                fprintf(stderr,
                        "Conflict: cell %d is both a snake head"
                        " and a ladder bottom\n",
                        head);
                return false;
            }
